...
def __setup_logging_ui_forwarding():
    ...
    if not any(
        [i.get_name() == LOGGING_HANDLER_NAME for i in pyinstaller_logger.handlers]
    ):
        handler = logging.StreamHandler(
            utils.ForwardToFunctionStream(send_message_to_ui_output)
        )
        handler.set_name(LOGGING_HANDLER_NAME)
        handler.setFormatter(
            logging.Formatter("%(relativeCreated)d %(levelname)s: %(message)s")
        )
...
def will_packaging_overwrite_existing(file_path, manual_name, one_file, output_folder):
    ...
    return packaging.will_packaging_overwrite_existing(
        file_path, manual_name, one_file, output_folder
    )
...
def start(open_mode):
    ...
    try:
        ...
        elif open_mode == UIOpenMode.USER_DEFAULT or (
            open_mode == UIOpenMode.CHROME and not chrome_available
        ):
