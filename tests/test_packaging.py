    ...
    with TemporaryDirectory() as build_directory:
        ...
        with TemporaryDirectory() as output_directory:
            with PythonScriptAsFile(filename, content) as script_path:
                pyinstaller_command = (
                    'pyinstaller --noconfirm --onedir --nowindowed "{}"'.format(
                        script_path
                    )
                )
                options = {
                ...
                predicted_output_folder = os.path.join(
                    output_directory, filename.split(".")[0]
                )
                assert os.path.exists(predicted_output_folder) and os.path.isdir(
                    predicted_output_folder
                )

                predicted_exe_location = os.path.join(
                    predicted_output_folder, filename.split(".")[0] + ".exe"
                )
                exe_output = subprocess.check_output(
                    [predicted_exe_location], cwd=predicted_output_folder
                )
